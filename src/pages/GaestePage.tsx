...
export default function GaestePage() {
  const [records, setRecords] = useState<Gaeste[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Gaeste | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Gaeste | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Gaeste | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, zimmerData] = await Promise.all([
        LivingAppsService.getGaeste(),
        LivingAppsService.getZimmer(),
      ]);
      setRecords(mainData);
      setZimmerList(zimmerData);
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Gaeste['fields']) {
    await LivingAppsService.createGaesteEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Gaeste['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGaesteEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGaesteEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Gäste"
      subtitle={`${records.length} Gäste im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zimmer_ref')}>
                <span className="inline-flex items-center gap-1">
                  Zimmer
                  {sortKey === 'zimmer_ref' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bemerkung_gast')}>
                <span className="inline-flex items-center gap-1">
                  Besondere Hinweise zum Gast
                  {sortKey === 'bemerkung_gast' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getZimmerDisplayName(record.fields.zimmer_ref)}</span></TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.bemerkung_gast ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={10} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Gäste. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
