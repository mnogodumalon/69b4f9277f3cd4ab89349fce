...
interface ZimmerDialogProps {
  ...
  defaultValues?: Zimmer['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function ZimmerDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: ZimmerDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'zimmer');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "zimmernummer": string | null, // Zimmernummer\n  "zimmertyp": LookupValue | null, // Zimmertyp (select one key: "doppelzimmer" | "suite" | "einzelzimmer" | "familienzimmer" | "apartment") mapping: doppelzimmer=Doppelzimmer, suite=Suite, einzelzimmer=Einzelzimmer, familienzimmer=Familienzimmer, apartment=Apartment\n  "kapazitaet": number | null, // Kapazität (Personen)\n  "etage": number | null, // Etage\n  "bemerkung_zimmer": string | null, // Bemerkungen\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Zimmer bearbeiten' : 'Zimmer hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="zimmertyp">Zimmertyp</Label>
            <Select
              value={lookupKey(fields.zimmertyp) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, zimmertyp: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="zimmertyp"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              type="number"
              value={fields.etage ?? ''}
              onChange={e => setFields(f => ({ ...f, etage: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="bemerkung_zimmer">Bemerkungen</Label>
            <Textarea
              id="bemerkung_zimmer"
              value={fields.bemerkung_zimmer ?? ''}
              onChange={e => setFields(f => ({ ...f, bemerkung_zimmer: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
